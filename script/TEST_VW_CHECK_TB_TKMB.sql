...
SELECT   short_name,
 ...
 WHERE   short_name = pck_glb_variables.get_short_name
         AND TO_DATE (kytt_den_ngay, 'DD/MM/YYYY') <>
             LAST_DAY(ADD_MONTHS ( TRUNC ( TO_DATE (pck_glb_variables.get_ky_chot,'DD/MM/RRRR'),'Year'),11))
...
SELECT   short_name,
 ...
 WHERE   short_name = pck_glb_variables.get_short_name
         AND TO_DATE (ngay_htoan, 'DD/MM/YYYY') <>
             LAST_DAY (TO_DATE (pck_glb_variables.get_ky_chot, 'DD/MM/RRRR'))
...
SELECT   short_name,
 ...
 WHERE   a.id = b.hdr_id AND a.short_name = pck_glb_variables.get_short_name
         AND (   b.thue_pn_nnt < 0
              ...
              OR b.thue_pn_cqt < 0)
 UNION
 -- Kiem tra du lieu to khai ton tai tren Header
 -- nhung khong co trong Detail
 SELECT   short_name,
         a.ROWID rid,
         'TB_TKMB_HDR' table_name,
         '021' err_id,
...
-- End of DDL Script for View TEST.VW_CHECK_TB_TKMB
