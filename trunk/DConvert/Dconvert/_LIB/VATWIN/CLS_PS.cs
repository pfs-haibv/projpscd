...
namespace DC.Vatwin
{
    class CLS_PS
    {
                                                ...
                                                Forms.Frm_QLCD p_frm_qlcd)
        {
            ...
            {
                DateTime _ky_ps_tu = new DateTime(p_ky_chot.Year-1,1,1);
                DateTime _ky_ps_den = p_ky_chot;
                ...
                // Biến lưu mô tả lỗi trong quá trình đọc file dữ liệu
                ...
                catch (Exception e)
                {
                    ...
                    _query +=
                        "call PCK_TRACE_LOG.prc_ins_log_vs('" + p_short_name + "', '"
                                                            + v_pck + "', 'N', '";
                    _query += e.Message.ToString().Replace("'", "\"") + "')";
                    _connOra.TransExecute(_query);
                    _connOra.TransRollBack();
                    return -1;
                }
                return _rowsnum;
            }
                ...
                try
                {
                    ...
                    if (_error_message.Length == 0)
                    {
                        _query = "call PCK_TRACE_LOG.prc_ins_log_vs('" + p_short_name + "', '"
                                                                    + v_pck + "', 'Y', null)";
                        _connOra_cntk.TransExecute(_query);
                        _connOra_cntk.TransCommit();
                    }
                    else
                    {
                        _query = "call PCK_TRACE_LOG.prc_ins_log_vs('" + p_short_name + "', '"
            ...
            using (CLS_DBASE.ORA _connOra_qdad = new CLS_DBASE.ORA(GlobalVar.gl_connTKTQ))
            {
                ...
                try
                {
                    ...
                    if (_error_message.Length == 0)
                    {
                        _query = "call PCK_TRACE_LOG.prc_ins_log_vs('" + p_short_name + "', '"
                                                                    + v_pck + "', 'Y', null)";
                        _connOra_qdad.TransExecute(_query);
                        _connOra_qdad.TransCommit();
                    }
                    else
                    {
                        _query = "call PCK_TRACE_LOG.prc_ins_log_vs('" + p_short_name + "', '"
