        ...
        {            
            using (CLS_DBASE.ORA _connOra_tkmb = new CLS_DBASE.ORA(GlobalVar.gl_connTKTQVATW))
            {
                ...
                try
                {
                    ...
                    _listFile.Clear();
                    _listFile = null;
                    //_query = "call PCK_CHUYENDOI_VAT.Prc_ChitietTKMB('" + p_short_name + "')";
                    //_connOra_tk10.TransExecute(_query);
                    //_query = "Select count(1) as rowscount from tb_pt where short_name='" + p_short_name + "'";
                    //DataTable _dt_rowcount;
                    //_dt_rowcount = _connOra_tk10.exeQuery(_query);
                    //foreach (DataRow _dr_rowcount in _dt_rowcount.Rows)
                    //{
                    //    _rowsnumn = Int32.Parse(_dr_rowcount["rowscount"].ToString().Trim());
                    //}
